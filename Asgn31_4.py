...
def CopyFilesWithGivenExtension(DirName1, DirName2, ext):
    Border = "*"*55
        ...
        for fname in FileName:
            ...
            if os.path.splitext(fname)[1].lower() == ext:
                ...
                if os.path.isfile(path1):
                    ...
                    print(f"Copied files with given extension {ext}: {fobj1} -> {fobj2}")
                    fobj.write(f"Copied files with given extension {ext}: {fobj1} -> {fobj2}"+"\n")      
...
def main():
    ...
    CopyFilesWithGivenExtension(sys.argv[1], sys.argv[2], sys.argv[3])
